...
| F-008 | EVT-VIEW / TRV-VIEW / ACT-VIEW / MOV-VIEW / RULE-VIEWER | **P1** ⏳ NOT FIXED | architectural gap (RLS + seed data) | ContactProfile (`/people/:contactId`) — "Viewer" role, all categories | The **Viewer** role (SOCIAL.md §1: "Passive visibility — control who can browse entries on your profile") appears non-functional for travel/activities/movies/events. `ContactProfile.js` queries `items` where `user_id = linkedUserId` then filters client-side by `item.visibilityRings.includes(ringLevel)` (or a higher ring). Two compounding gaps: (1) **RLS** — the `items` SELECT policies only grant cross-user reads via an accepted collaborator row or a recommendation-recipient row; there is **no ring-visibility policy**, so a pure "viewer" (no collab/rec relationship) gets zero rows back regardless of `visibilityRings`. (2) **seed data** — even for users who *do* have a readable relationship (e.g. Mike is an accepted collaborator on Jason's Super Bowl LVIII and ACL Festival), those seeded travel/activities/movies/events items never set `visibilityRings` (empty array), so ContactProfile's filter excludes them too, rendering "Nothing shared yet". `visibilityRings` itself **is** wired up end-to-end in `ItemForm`/`PeopleField`/`ShareWithSection`/`PrivacyIndicator`/`ReadOnlySocialSection`/`EntryView` — only cars/homes seed data sets it (`[1,2,3,4]`). | As Mike, visit `/people/<jason-contact-id>` — "Nothing shared yet" despite Mike being an accepted collaborator with overlays on 2 of Jason's entries. DB: `select * from pg_policies where tablename='items'` shows only "Collaborators can read shared entries" and "Recipients can read recommended entries" (migration 009) — no ring-based policy. | **Not auto-fixed** (same risk class as F-007 — a new RLS policy). Needs a scope decision: (a) add a ring-visibility SELECT policy on `items` (e.g. `EXISTS` against `contacts`/ring level, mirroring `get_my_ring_recommender_ids`), and (b) set `visibilityRings` on relevant seeded travel/activities/movies/events entries (as cars/homes already do) so the Viewer journeys have real data to show. Surfaced to user for go/no-go before implementing. |
